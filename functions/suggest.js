const CATEGORY_DESCRIPTIONS = {
  ...
  const isRateLimited = (ip) => {
    ...
    if (minuteHits.length > MAX_PER_MINUTE) return "Çok kararsız gibisin... 😵‍💫 Senin için en mükemmel hediyeyi ararken yapay zekamızın biraz başı döndü! Hediye perilerimizin nefes nefese kalmaması için dakikada sadece 5 kez çarkı çevirebiliyoruz. Derin bir nefes al, kısa bir mola ver ve birazdan tekrar dene!";
  ...
  const callOpenRouter = async (apiKey, prompt) => {
    const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${apiKey}`,
        "HTTP-Referer": "https://hediye-carki.pages.dev",
  ...
  // ✅ Cloudflare Pages Functions formatı (Netlify'dan tek fark burası)
  export async function onRequestPost(context) {
    ...
    if (rateLimitMsg) {
      return new Response(JSON.stringify({ error: rateLimitMsg }), { status: 429, headers });
    }
    ...
    if (!groqKey && !openrouterKey) {
      return new Response(
        JSON.stringify({ error: "API key yapılandırılmamış. Cloudflare ortam değişkenlerini kontrol edin." }),
        { status: 500, headers }
      );
    }
